...
def generate_hills_and_valleys(filename, size=10, resolution=0.2, noise_scale=0.18, noise_amplitude=20):
    """
    Generate a terrain with rolling hills and valleys using Perlin noise only.
    """

    vertices = []
    faces = []

    # Create the top surface vertices
    for i in range(int(size / resolution)):
        for j in range(int(size / resolution)):
            x = -size / 2 + i * resolution
            y = -size / 2 + j * resolution
                        
            # Perlin noise for hills and valleys
            z = pnoise2(x * noise_scale, y * noise_scale, octaves=4) * noise_amplitude

            vertices.append([x, y, z])

    # Create flat base vertices
    for i in range(int(size / resolution)):
        for j in range(int(size / resolution)):
            x = -size / 2 + i * resolution
            y = -size / 2 + j * resolution
    ...
    for i in range(grid_size - 1):
        for j in range(grid_size - 1):
            ...
            # Bottom surface
            faces.append([base_offset + a, base_offset + c, base_offset + b])
            faces.append([base_offset + c, base_offset + d, base_offset + b])

    # Side walls
    for i in range(grid_size - 1):
        for j in range(grid_size):
            top1 = i * grid_size + j
            top2 = (i + 1) * grid_size + j
            base1 = base_offset + top1
            base2 = base_offset + top2

            faces.append([top1, base1, top2])
            faces.append([top2, base1, base2])

    # Write to OBJ
    write_to_obj(filename, vertices, faces)

# Generate the OBJ file
generate_hills_and_valleys('./shapes/hills_and_valleys.obj')
