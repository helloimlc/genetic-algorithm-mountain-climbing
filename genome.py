...
class Genome():
    ...
    def copy_structure_and_randomize_motors(fixed_dna):
        ...
        motor_keys = ["control-waveform", "control-amp", "control-freq"]
    ...
    def apply_waveform_ratio(dna, sine_ratio=0.7):
        ...
        for gene in dna:
            if np.random.rand() < sine_ratio:
                gene[waveform_index] = np.random.uniform(0.51, 0.9999)  # Sine
            else:
                gene[waveform_index] = np.random.uniform(0.0, 0.49)     # Pulse
        return dna

    @staticmethod
    def clamp_amp(dna, fixed_amp=0.2):
        spec = Genome.get_gene_spec()
    ...
    @staticmethod
    def clamp_freq(dna, fixed_freq=1.0):
        spec = Genome.get_gene_spec()
