...
class Genome():
    ...
    @staticmethod
    def get_symmetric_genome(gene_length, gene_count):
        assert gene_count % 2 == 0, "Gene count must be even for symmetry"
        half = gene_count // 2
        base_genes = [Genome.get_random_gene(gene_length) for _ in range(half)]
        mirrored_genes = []

        spec = Genome.get_gene_spec()
        flip_index = spec["joint-origin-xyz-1"]["ind"]  # flip x position

        for g in base_genes:
            g_copy = np.copy(g)
            g_copy[flip_index] *= -1  # mirror left-right by flipping X
            mirrored_genes.append(g_copy)

        return base_genes + mirrored_genes

    @staticmethod
    def randomize_phase(dna):
        spec = Genome.get_gene_spec()
        phase_index = spec["control-phase"]["ind"]
        for gene in dna:
            gene[phase_index] = random.uniform(0, np.pi * 2)
        return dna
    ...
    def get_gene_spec():
        gene_spec =  {"link-shape":{"scale":1}, 
            ...
            "link-radius": {"scale":1.0}, # Change this for thin-long experiment
            ...
            "control-phase": {"scale": np.pi * 2}  # Ranges from 0 to 2π
            }
        ind = 0
    ...
    def genome_to_links(gdicts):
        ...
        for gdict in gdicts:
            ...
            link = URDFLink(name=link_name, 
                            ...
                            control_phase=gdict["control-phase"])
            links.append(link)
    ...
    # @staticmethod
    # def copy_structure_and_randomize_motors(fixed_dna):
    #     spec = Genome.get_gene_spec()
    #     motor_keys = ["control-waveform", "control-amp", "control-freq", "control-phase"]

    #     new_dna = []
    #     for gene in fixed_dna:
    #         new_gene = np.copy(gene)
    #         for key in motor_keys:
    #             ind = spec[key]["ind"]
    #             new_gene[ind] = np.random.rand()
    #         new_dna.append(new_gene)
    #     return new_dna

    @staticmethod
    def apply_waveform_ratio(dna, sine_ratio=0.7):
        spec = Genome.get_gene_spec()
        waveform_index = spec["control-waveform"]["ind"]

        for gene in dna:
            if np.random.rand() < sine_ratio:
                gene[waveform_index] = np.random.uniform(0.26, 0.5)  # True SINE
            else:
                gene[waveform_index] = np.random.uniform(0.0, 0.25)  # True PULSE
        return dna

    @staticmethod
    def clamp_amp(dna, fixed_amp=0.2): # Adjust for amplitude experiment
        spec = Genome.get_gene_spec()
        amp_index = spec["control-amp"]["ind"]
        for gene in dna:
            gene[amp_index] = fixed_amp
        return dna

    @staticmethod
    def clamp_freq(dna, fixed_freq=1.0): # Adjust for frequency experiment
        spec = Genome.get_gene_spec()
        freq_index = spec["control-freq"]["ind"]
        for gene in dna:
            gene[freq_index] = fixed_freq
        return dna
...
class URDFLink:
    def __init__(self, name, parent_name, recur, 
        ...
        self.link_shape = link_shape  # new shape gene value
    ...
    def to_link_element(self, adom):
        ...
        # === Create shape based on link_shape value ===
        # Assume cylinder unless allowed and shape gene is high enough
        force_cylinder = self.link_shape_tag_override if hasattr(self, "link_shape_tag_override") else False

        if force_cylinder or self.link_shape < 0.33:
            ...
            size = f"{self.link_radius} {self.link_radius} {self.link_length}"
            shape_tag.setAttribute("size", size)
        else:
        ...
        # === Collision tag === (clone shape for collision)
        coll_tag = adom.createElement("collision")
        c_geom_tag = adom.createElement("geometry")
        c_geom_tag.appendChild(shape_tag.cloneNode(True))
